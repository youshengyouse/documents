...
本文档概述了 Next.js 中**所有**的文件夹和文件约定，并提供了组织项目的建议。
...
<Image
  alt="路由段到路径段的映射"
  srcLight="/docs/nextjs/light/top-level-folders.png"
  srcDark="/docs/nextjs/light/top-level-folders.png"
  width="1600"
  height="525"
/>{" "}
...
|                                                                    |                            |
| ------------------------------------------------------------------ | -------------------------- |
| [`app`](/docs/app/building-your-application/routing)               | App 路由                   |
| [`pages`](/docs/pages/building-your-application/routing)           | Pages 路由                 |
...
Next.js 对如何组织和共置项目文件**没有强制要求**。但它确实提供了几个功能来帮助您组织项目。
...
在`app`目录中，嵌套文件夹定义了路由结构。每个文件夹代表一个路由段，映射到 URL 路径中的相应段。
...
> - 如果不使用私有文件夹，了解 Next.js[特殊文件约定](/docs/app/getting-started/project-structure#routing-files)有助于防止意外的命名冲突。
...
Next.js 支持将应用程序代码(包括`app`)存储在可选的[`src`文件夹](/docs/app/api-reference/file-conventions/src-folder)中。这将应用程序代码与主要位于项目根目录中的项目配置文件分开。
